                            ...
                            @endif
                                <a href="/moshaver/editfile/{{$file->id}}">
                                    ...
                                    <div class="col_left_box" style="float: left;">
                                        <p>{{$file->name ? (($file->kind_type == 'sell' ? 'خریدار' : 'مستجر') .' '. $file->name) :  'عنوان فایل'}}</p>
                                        <p class="addfilelist_userid_file"> شماره تلفن : <span>{{$file->phone }}</span></p>
                                        <p class="addfilelist_price">
                                            @if($file->price)
                                             قیمت ثبت شده : {{$file->price}} میلون تومان
                                            @else
                                            قیمت : بدون قیمت
                                            @endif
                                        </p>
                                        <p class="addfilelist_area">
                                            ...
                                            <span style="font-size: 10px;">{{$file->area ?  $file->area : 0}} متراژ ثبت شده</span>
                                        </p>
                                        <div class="addfilelist_colbottom">
                                            <div class="row">
                                                ...
                                                <div class="col-md-2 col-4">
                                                    <span class="bottomcol_span"> آسانسور {{$file->elevator == null ? '-' : ($file->elevator == 1 ? 'ثبت' : '-')}}</span>
                                                </div>
                                                ...
                                                <div class="col-md-2 hideinphone">
                                                    <span class="bottomcol_span"> پارکینگ {{$file->parking == null ? '-' : ($file->elevator == 1 ? 'ثبت' : '-')}}</span>
                                                </div>

                                                <div class="col-md-2 hideinphone">
                                                    <span class="bottomcol_span"> بالکن {{$file->balcony == null ? '-' : ($file->elevator == 1 ? 'ثبت' : '-')}}</span>
                                                </div>
                                            </div>    
                                        </div>
